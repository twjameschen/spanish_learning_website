...
export const grammarLessonSchema = z.object({
   ...
   * **作者註記，不是驗證結果，也不要拿去顯示。**
   *
   * 意思是：本課示範文法規則的例句只用了本課與前置課教過的文法。
   * （單字表的 exampleEs 不受此限，那邊實用優先。）
   *
   * 課文頁曾經拿它畫一顆綠色的「例句嚴格分級」徽章。那顆徽章有兩個問題：
   * 41 課**全部**宣告 true，所以它永遠是亮的、分辨不出任何東西；
   * 而且它把一個沒有任何東西查核的自述，呈現得像已經驗證過的屬性 ——
   * 規格的內容誠實原則不允許這樣。
   *
   * 有試過用 `verbForms.ts` 的變位反查索引機械驗證，**做不到**：
   * 例句裡的 `de`（dar 的現在虛擬式）、`cómo`（折重音後撞到 como）
   * 這類同形字會產生大量假陽性，而 A0 的 `Soy`／`Tengo` 出現在
   * 「現在式」那一課之前其實是對的 —— `a0-ser-estar` 本來就在教那些形式。
   * 要分辨「當成系統教的時態」與「當成固定說法教的個別形式」，
   * 資料裡沒有足夠的資訊。寫作準則寫在 README，這個欄位留著給作者自己看。
   */
