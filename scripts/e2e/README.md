...
| `help-and-settings.mjs` | 聽力題的兩階求助（會塞一個假的西班牙文語音進去，否則容器裡量不到有語音的那條路徑）、設定面板改得動而且真的有作用、遮罩鋪滿視窗 |
| `dashboard.mjs` | 空狀態、熱力圖、XP 折線、詞性雷達、最弱十字、匯出匯入 |
| `shortcuts.mjs` | 數字鍵選答案、Space 翻閃卡、`?` 說明面板、輸入框裡不搶鍵盤 |
| `contrast.mjs` | 9 個頁面 × 淺／深兩種模式的 WCAG AA 文字對比，另外量設定面板與聽力提示這兩個「開起來才存在」的狀態（漸層背景會跳過並回報） |
